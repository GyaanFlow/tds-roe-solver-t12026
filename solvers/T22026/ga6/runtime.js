...
export function wrapSolverModule(mod) {
  ...
  if (typeof id !== 'string' || !id) {
    throw new Error('GA6 solver module is missing a valid id.');
  }
  if (typeof title !== 'string' || !title) {
    throw new Error(`GA6 solver ${id} is missing a valid title.`);
  }
  if (typeof solveImpl !== 'function') {
    throw new Error(`GA6 solver ${id} is missing a solve() function.`);
  }
