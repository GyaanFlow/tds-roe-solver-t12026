...
export async function solve(email) {
  ...
  const guide = [
    ...
    `>`,
    `> This is deliberately not a visual matching exercise. Handles and transaction keys contain composed`,
    `> versus decomposed accents, fullwidth forms, mixed-script Cyrillic look-alikes, and explicitly listed`,
    `> invisible code points. The ledger also contains exact transport replays, superseded revisions,`,
    `> equal-revision corrections, business-level duplicates, and integer values too large for safe`,
    `> floating-point arithmetic.`,
    `>`,
    `> **What makes the certificate auditable:**`,
    ...
    `>`,
    '> **Submission:** `{"suspicious_account_ids":["acct-..."],"accepted_event_ids":["evt-..."],"net_minor_units":"...","evidence_digest":"fnv1a32:..."}`.',
    `> ID-array order is ignored. The four components receive independent partial credit, but malformed`,
    `> types, duplicate IDs, extra keys, or oversized submissions are rejected.`,
    ``,
    `### ⚡ Dynamic Interactive Direct Solver (Unique for ${norm})`,
    ``,
    `> 🔧 **Backup method:** if this in-browser tool fails to load or run, the exact same algorithm`,
    '> is also available as a standalone Node.js script: `solvers/T22026/roe/offline-scripts/q2-unicode-ledger.offline.mjs`.',
    '> Run `node q2-unicode-ledger.offline.mjs your-artifact.json` — verified to produce byte-identical output.',
    ``,
