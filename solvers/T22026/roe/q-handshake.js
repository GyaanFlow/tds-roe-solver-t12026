...
export async function solve(email) {
  ...
  const guide = [
    ...
    `> Shake hands with **N classmates** (N is shown on your exam page). A handshake takes two codes,`,
    ...
    `>`,
    `> **How it works:**`,
    `> 1. Pick a classmate. Run \`code("c", their_email)\` and send them the result. That is your`,
    `>    **challenge**.`,
    `> 2. They run \`code("r", your_challenge)\` with *their* key and send it back. That is the`,
    `>    **response**. Only they can produce it.`,
    `> 3. Save the row, then do the same for them so you both get credit.`,
    `> 4. Repeat until you have N different classmates.`,
    `>`,
    `> **Exact recipe:** HMAC-SHA256, keeping the first *tagLength* characters of the lowercase hex`,
    `> digest. The key and the message are both plain UTF-8 text — do not hex-decode the key. Emails`,
    '> are lowercased and trimmed. The separator is one `|` with no spaces, so the messages are',
    '> `"c|" + their_email` and `"r|" + challenge`.',
    `>`,
    `> **What to submit:** one row per classmate — a JSON array of`,
    '> `{"peer":"classmate@ds.study.iitm.ac.in","challenge":"...","response":"..."}` objects. Partial',
    `> work counts, so submit whatever you have. Worth 5 marks, scaled by verified classmates (N for`,
    ...
    ``,
    `> 🔧 **Backup method:** if this in-browser toolkit fails to load or run, the exact same`,
    '> HMAC-SHA256 recipe is also available as a standalone Node.js script: `solvers/T22026/roe/offline-scripts/q5-handshake.offline.mjs`',
    '> (`challenge`/`respond`/`assemble` subcommands) — verified to produce byte-identical output.',
    ``,
