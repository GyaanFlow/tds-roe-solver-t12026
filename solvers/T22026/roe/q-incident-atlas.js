...
export async function solve(email) {
  ...
  const guide = [
    ...
    `> Recover the incident records from the pixels, georegister them to directed road edges, and submit`,
    `> an earliest valid route that visits the mandatory checkpoint.`,
    `>`,
    `> ⚠️ This is a combined raster-forensics, affine-coordinate, and time-dependent routing task. Edge`,
    `> direction, the published turn restriction, half-open incident windows, and optional waiting all`,
    `> affect the certificate.`,
    `>`,
    `> **Submission:** exactly three fields — \`incidents\`, \`route_edge_ids\`, \`arrival_seconds\` (e.g.`,
    '> `{"incidents":[{"edge_id":"H00E","effect":"CLOSED","window_id":"W1"}],"route_edge_ids":["H00E"],"arrival_seconds":123}`).',
    `> Incident order does not matter; route edge order does. Decoded incidents and the route certificate`,
    `> earn separate credit.`,
    ``,
    `### ⚡ Dynamic Interactive Direct Solver (Unique for ${norm})`,
    ``,
    `> 🔧 **Backup method:** if this in-browser tool fails to load or run (JS disabled, a browser`,
    `> extension conflict, unexpected error), the exact same algorithm is also available as a`,
    '> standalone Node.js script: `solvers/T22026/roe/offline-scripts/q1-incident-atlas.offline.mjs`.',
    '> Run `node q1-incident-atlas.offline.mjs your-artifact.json` — verified to produce byte-identical output.',
    ``,
